...
function renderQuickPrompts() {
  if (!quickPromptEl) return;
  if (!QUICK_PROMPTS.length) {
    quickPromptEl.innerHTML = '<span class="muted">暂无推荐</span>';
    return;
  }
  quickPromptEl.innerHTML = QUICK_PROMPTS.map((p) => `
    <button class="prompt-chip" data-prompt="${escapeAttr(p)}">${escapeHtml(p)}</button>
  `).join('');
  quickPromptEl.querySelectorAll('.prompt-chip').forEach((btn) => {
    btn.onclick = () => applyPrompt(btn.getAttribute('data-prompt'));
  });
}

function applyPrompt(promptText) {
  if (!promptText || !chatInputEl) return;
...
function renderAnswerWithSources(answer, sources, metaInfo) {
  const chips = (sources || []).map(s => {
    ...
    const label = escapeHtml(s.source || '未知来源');
    const raw = escapeAttr(s.source || '');
    return `<span class="chip" data-doc="${raw}">${label}${score}</span>`;
  }).join(' ');
  const meta = metaInfo || {};
  const toolLine = Array.isArray(meta.used_tools) && meta.used_tools.length
    ? meta.used_tools.map(t => escapeHtml(t)).join(', ')
    : '无';
  const parts = [];
  if (meta.intent) parts.push(`意图: ${escapeHtml(meta.intent)}`);
  parts.push(`工具: ${toolLine}`);
  if (meta.latency) parts.push(`延迟: ${Math.round(meta.latency)} ms`);
  const metaLine = parts.join(' · ');
  return `
    <div class="message-text">${formatRichText(answer)}</div>
    <div class="sources-row"><span class="label">来源</span>${chips || '<span class="muted">无</span>'}</div>
    <div class="sources-row small">${metaLine || ''}</div>
  `;
}
...
async function loadHistory() {
  ...
  try {
    ...
    history.forEach(msg => {
      ...
      appendMessage(role, formatRichText(msg.content || ''));
    });
...
async function ask() {
  ...
  const top_k = parseInt(document.getElementById('topk').value, 10) || 4;
  ...
  const payload = { query, top_k, need_tool, session_id: chatSessionId };
...
async function handleStreamingRequest(payload, assistantBubble, token, signal) {
  ...
  while (true) {
    ...
    for (const piece of pieces) {
      ...
      answer += piece;
      if (textEl) textEl.innerHTML = formatRichText(answer);
    }
  }
  if (meta) {
    finalizeAssistantBubble(assistantBubble, answer, meta.sources, { intent: meta.intent, used_tools: meta.used_tools });
  } else {
    if (textEl) textEl.innerHTML = formatRichText(answer || '生成完成');
  }
}
...
async function handleStandardRequest(payload, assistantBubble, token, signal) {
  ...
  const data = await resp.json();
  finalizeAssistantBubble(assistantBubble, data.answer, data.sources, {
    intent: data.intent,
...
function finalizeAssistantBubble(bubble, answer, sources, meta) {
  setBubbleContent(bubble, renderAnswerWithSources(answer, sources, meta));
  bindChipClicks(bubble);
}
...
function initPage() {
  ...
  quickPromptEl = document.getElementById('quickPrompts');
  renderQuickPrompts();
  if (chatInputEl) {
